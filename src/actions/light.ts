  ...
  public async onKeyUp({ context, payload }: KeyUpEvent<DeviceSettingsInterface>): Promise<void> {
    ...
    try {
      ...
      switch (behaviour) {
        ...
        case 'more':
          const currentLevel = deviceStatus.components?.main?.switchLevel?.level?.value as number
        ...
        case 'less':
          const level = deviceStatus.components?.main?.switchLevel?.level?.value as number
